...
- **G8.** Agent usable **without** the IDE (`-javaagent:runtimerocket-agent.jar` + a standalone / fixture-written `runtimerocket.xml`). The IDE compile path sends class bytes over the loopback protocol and does **not** require that file. Forked launchers that never see `JavaProgramPatcher` require a manual `-javaagent` (v1 limitation).
...
platform.type=IC
# Compile against the API floor; pluginVerifier checks IC-2024.3 (2026.2 is targeted, not verified)
platform.version=2024.3
plugin.since.build=243
plugin.until.build=262.*
```

**Compatibility window (normative).** Oldest supported IDE is IntelliJ IDEA **2024.3** (build 243). Newest *targeted* IDE is **2026.2** (build 262). Newest *verified* IDE is **2024.3** (`pluginVerifier` = `IC-2024.3`). `until-build=252.*` is forbidden — that is 2025.2 and current IDEs refuse it. Gradle wrapper is **9.0+** because Platform Plugin 2.18.1 requires it. 2026.2 verification is a follow-up: IC installers ended at 2025.3; the unified `IntellijIdea` type needs a pinned 2026.2.x and a large extra download.
...
Analog of JRebel’s public `rebel.xml`. **Standalone / fixture-generated** — write it yourself or emit it from Gradle (see `:fixtures:two-module`). Place it on the application classpath or pass `config=`. The IDE compile path sends class bytes over the loopback protocol and does **not** require this file (0.1.0 has no `RocketXmlGenerator`).
